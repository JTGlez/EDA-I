...
int main(){
    ...
    void llenarArreglo(struct pelicula arreglo[TAM]) { //Se copia la dirección del arreglo, no el arreglo en sí.
        int iesimo, enesimo; //Variables de control.
        for (iesimo=0 ; iesimo<TAM ; iesimo++) {
            struct pelicula movie; //Variable temporal tipo struct. Todo lo que contenga se copiará a la variable dummy "movie".
            printf("####### Película %d #######\n", iesimo+1);
